...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("get_links", [](Index<float> &index, hnswlib::labeltype label) {
            auto *alg = index.appr_alg;
        
            std::unordered_map<int, std::vector<hnswlib::labeltype>> result;
        
            hnswlib::tableint internal_id = alg->label_lookup_.at(label);
            int max_level = alg->element_levels_[internal_id];
        
            for (int level = 0; level <= max_level; level++) {
                std::vector<hnswlib::labeltype> neighbors;
        
                hnswlib::linklistsizeint *link_ptr = (level == 0)
                    ? alg->get_linklist0(internal_id)
                    : alg->get_linklist_at_level(internal_id, level);  // <-- correct for level > 0

                int size = alg->getListCount(link_ptr);
                hnswlib::tableint *data = reinterpret_cast<hnswlib::tableint*>(link_ptr + 1);

                for (int i = 0; i < size; i++) {
                    hnswlib::tableint nb = data[i];
                    neighbors.push_back(alg->getExternalLabel(nb));
                }
        
                result[level] = neighbors;
            }
